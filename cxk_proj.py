...
import pygame, sys, random # 导入pygame包（游戏制作）、sys（程序与解释器交互）、random（随机数）

# different images for the skier depending on his direction
skier_images = ["skier_down.png", "skier_right1.png", "skier_right2.png",
                 "skier_left2.png", "skier_left1.png"] # 图

# class for the skier sprite  滑雪者类
class SkierClass(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("skier_down.png") # 加载直行图片
        self.rect = self.image.get_rect()
        self.rect.center = [320, 100] #中心
        self.angle = 0 # 角度
        
    def turn(self, direction): 
        # load new image and change speed when the skier turns # 载入新图并改变速度
        self.angle = self.angle + direction
        if self.angle < -2:  self.angle = -2
        if self.angle >  2:  self.angle =  2 
        center = self.rect.center
        self.image = pygame.image.load(skier_images[self.angle]) # 加载对应的图片改变对应的速度
        self.rect = self.image.get_rect()
    ...
    def move(self, speed):
        ...
        if self.rect.centerx < 20:  self.rect.centerx = 20   ###限制边界
        if self.rect.centerx > 620: self.rect.centerx = 620  ###限制边界
        
# class for obstacle sprites (trees and flags)  # 障碍物类
class ObstacleClass(pygame.sprite.Sprite):
    def __init__(self, image_file, location, type):
        ...
        self.type = type # 判断是树还是旗
        self.passed = False # 是否经过
                   
    def scroll(self, terrainPos):
        self.rect.centery = self.location[1] - terrainPos

# create one "screen" of terrain: 640 x 640  创建窗口
# use "blocks" of 64 x 64 pixels, so objects aren't too close together
def create_map(start, end):
    obstacles = pygame.sprite.Group()
    locations = []
    gates = pygame.sprite.Group()
    for i in range(10):                 # 10 obstacles per screen 每个场景10个障碍物
        row = random.randint(start, end)
        col = random.randint(0, 9)
        location  = [col * 64 + 20, row * 64 + 20] #center x, y for obstacle
        if not (location in locations):        # prevent 2 obstacles in the same place  避免重复
